...
export interface ParsedTweetAuthor {
  ...
  following?: boolean;
  lang?: string;
  /** 账号计数与形态（Jev 判定层的 state 用；同一条响应里就有，不发额外请求）。 */
  followers?: number;
...
interface ParsedUserResult {
  ...
  following?: boolean;
  blocking?: boolean;
  /** 账号计数与形态字段：Jev 判定层的 state 需要，时间线载荷里本来就有。 */
  followers?: number;
  followingCount?: number;
  statuses?: number;
  createdAt?: string;
...
function parseUserResult(result: Json): ParsedUserResult | null {
  ...
  // 2026-09 起计数走 relationship_counts / tweet_counts，旧版在 legacy 里并存。
  const counts = result?.relationship_counts;
  const tweets = result?.tweet_counts;
  const followers = counts?.followers ?? legacy?.followers_count;
  const followingCount = counts?.following ?? legacy?.friends_count;
  const statuses = tweets?.tweets ?? legacy?.statuses_count;
  const createdAt = core?.created_at ?? legacy?.created_at;
  const avatarUrl = result?.avatar?.image_url;
  const hasCustomAvatar =
    typeof avatarUrl === 'string' ? !avatarUrl.includes('default_profile_images') : undefined;
  const verified = result?.verification?.verified ?? result?.is_blue_verified ?? legacy?.verified;
  return {
    handle,
    ...
    ...(typeof statuses === 'number' ? { statuses } : {}),
    ...(typeof createdAt === 'string' ? { createdAt } : {}),
    ...(typeof hasCustomAvatar === 'boolean' ? { hasCustomAvatar } : {}),
  ...
  try {
    ...
    return {
      ...
      author: {
        ...
        ...(typeof user.followingCount === 'number' ? { followingCount: user.followingCount } : {}),
        ...(typeof user.statuses === 'number' ? { statuses: user.statuses } : {}),
        ...(typeof user.createdAt === 'string' ? { createdAt: user.createdAt } : {}),
        ...(typeof user.hasCustomAvatar === 'boolean'
