 ...
 * 快照的拉取/校验逻辑在 @qingliu/community-lists（纯函数），
...
import {
  ...
  type StoredSnapshot,
  type SyncSource,
} from '@qingliu/community-lists';
import bundledSnapshotJson from '../../../../community/lists/official.json';

/**
 * 社区名单服务地址。**默认留空 = 不启用远程服务**。
 *
 * 为什么默认关掉：上游那台 Worker（feedsieve-api.chendahuang.com）已经随项目停运
 * （实测 2026-09-19 连接失败），而原仓库已转私。与其把扩展指向一台死掉、还会让
 * CWS 审核多问一句的服务器，不如把这里做成一个干净的接缝：
 *
 *   · 留空：只用**内置快照**（`community/lists/official.json`）+ 关键词库 + AI 判定层；
 *     不发任何网络请求，也不申请多余的 host 权限。
 *   · 自部署：见 `apps/community-api`（Cloudflare Workers + D1），
 *     然后把地址填在这里，或构建时注入 `WXT_COMMUNITY_API_BASE`。
 *
 * 这也是商业化方案里「客户端开源、服务端收费」要插进去的那条缝。
 */
const configuredBase = ((import.meta.env?.WXT_COMMUNITY_API_BASE as string | undefined) ?? '').trim();
export const COMMUNITY_API_BASE = configuredBase;
/** 远程服务是否可用；关闭时所有上报/同步直接短路，零网络请求。 */
export const COMMUNITY_SERVICE_ENABLED = configuredBase.length > 0;

/** 快照来源：仅在配置了服务地址时才存在（最小 host 权限原则）。 */
export const COMMUNITY_SYNC_SOURCES: SyncSource[] = COMMUNITY_SERVICE_ENABLED
