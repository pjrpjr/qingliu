...
export const UI_COPY = {
  zh: {
    ...
    personalConfigInvalid: '这不是可导入的清流个人配置',
    ...
    jevEnabled: '启用 AI 判定',
    jevEnabledHint:
      '名单、指纹、词库都没认出时，把账号资料交给 Jev 判一次。只出黄框，绝不自动拉黑；需要你自己的 Typesafe API key。',
    jevApiKey: 'API key',
    jevApiKeyHint: '只存在本机浏览器里，不上传任何服务器。',
    jevReady: '已启用',
    jevNotConfigured: '待填 key',
    jevDisabled: '未启用',
  ...
  en: {
    ...
    personalConfigInvalid: 'This is not a valid Qingliu personal config',
    ...
    jevTitle: 'AI verdict (Jev)',
    jevEnabled: 'Enable AI verdict',
    jevEnabledHint:
      'When lists, fingerprints and keywords all miss, ask Jev once about the account. It only adds a review frame — never blocks on its own. Requires your own Typesafe API key.',
    jevApiKey: 'API key',
    jevApiKeyHint: 'Stored only in this browser. Never uploaded to any server.',
    jevReady: 'Active',
    jevNotConfigured: 'Key needed',
    jevDisabled: 'Off',
...
export function localizedDetectionReason(
  ...
  if (detection.ruleId?.startsWith('jev-')) {
    // AI 判定：类别翻成人话，并保留置信度（`AI 判定（87%）` 里的数字）。
    const label = categoryLabel(detection.ruleId.slice('jev-'.length), language);
    const percent = detection.reason.match(/(\d+)\s*%/)?.[1];
    if (language === 'zh') {
      return percent ? `AI 疑似${label}（${percent}%）` : `AI 疑似${label}`;
    }
    return percent ? `AI: likely ${label} (${percent}%)` : `AI: likely ${label}`;
  }
  if (language === 'zh') {
