import { contentFingerprint, detect, toHandleSet } from '@qingliu/detector';
import type { CommunityEntry } from '@qingliu/community-lists';
import {
  contextFromPath,
  extractFeedItem,
  parseXApiResponse,
...
import {
  JevDecisionCache,
  accountCacheKey,
  thresholdsForStrength,
  verdictToDetection,
  type JevAccountInput,
  type JevVerdict,
} from '@qingliu/jev-detect';
import {
  DEFAULT_JEV_SETTINGS,
  getJevSettings,
  isJevActive,
  subscribeJevSettings,
  type JevSettings,
...
export default defineContentScript({
  ...
  main() {
    ...
    /**
     * handle -> 账号计数与形态（粉丝/关注/推文数/注册时间/是否默认头像/蓝标）。
     * 这些字段本来就在时间线的 XHR 载荷里，白捡的 —— Jev 判定层的 state 要用，
     * 而离线评测标定阈值时用的正是同一组字段（口径必须一致）。
     */
    ...
    /** Jev 语义层设置（默认关闭：关闭时行为与上游原版完全一致）。 */
    let jevSettings: JevSettings = { ...DEFAULT_JEV_SETTINGS };
    /** 会话内判定缓存：同一账号不重复问（key 带内容指纹，换话术会失效）。 */
    const jevCache = new JevDecisionCache();
    /** 待判队列：handle -> 页面上下文（凑够一批再发，吃扇出的成本优势）。 */
    const jevQueue = new Map<string, JevQueueItem>();
    let jevFlushTimer: number | undefined;
    ...
    void refreshJevSettings();
    subscribeJevSettings((settings) => {
      jevSettings = settings;
      // 开关或阈值变了：清掉待判队列并重扫，让新档位立刻生效。
      jevQueue.clear();
        ...
        try {
          ...
          for (const tweet of parsed.tweets ?? []) {
            ...
            }
            {
              ...
              const metrics = {
                ...(typeof author.followers === 'number' ? { followers: author.followers } : {}),
                ...(typeof author.followingCount === 'number'
                  ? { following: author.followingCount }
                  : {}),
                ...(typeof author.statuses === 'number' ? { statuses: author.statuses } : {}),
                ...(typeof author.createdAt === 'string' ? { createdAt: author.createdAt } : {}),
                ...(typeof author.hasCustomAvatar === 'boolean'
                  ? { hasCustomAvatar: author.hasCustomAvatar }
                  : {}),
                ...(typeof author.verified === 'boolean' ? { verified: author.verified } : {}),
              };
              if (Object.keys(metrics).length > 0) {
                const previous = metricsCache.get(handle);
                if (JSON.stringify(previous) !== JSON.stringify(metrics)) {
                  metricsCache.set(handle, metrics);
                  if (metricsCache.size > BIO_CACHE_MAX) {
                    const oldest = metricsCache.keys().next().value;
                    if (oldest !== undefined) metricsCache.delete(oldest);
                  }
                  // 计数到位后让该账号的卡片重新评估（可能刚好跨过 AI 阈值）
                  dirtyHandles.add(handle);
                }
              }
            }
    ...
    function scanOne(
      ...
      if (!detection) {
        // 本地三层（名单 / 指纹 / 词库）全空：交给 Jev 语义层异步判。
        // 注意这是**最后**一层 —— 前面任何一层命中都不会走到这里，
        // 所以 AI 永远不会覆盖名单或用户自己的规则。
        enqueueJevJudgment(article as HTMLElement, handle, input, evidence);
        attachManualAction(article as HTMLElement, handle, evidence);
        return;
      }
    ...
    // ------------------------------------------------------------ Jev 语义层
    //
    // 位置很关键：这一层排在名单 / 指纹 / 词库**之后**，只处理前三层全部 miss 的账号。
    // 它只产出黄框，永远不自动拉黑（`classifyDetection` 对 source==='ai' 返回 'review'）。
    // 阈值按页面既有的「识别强度」档位取，数字来自 jev/RESULTS.md 的实测扫描。

    interface JevQueueItem {
      handle: string;
      account: JevAccountInput;
      article: HTMLElement;
      evidence: BlockEvidence;
    }

    /** 待判账号攒到这么多就先发一批；不足则等 JEZ_FLUSH_DELAY_MS。 */
    const JEV_BATCH_SIZE = 10;
    const JEV_FLUSH_DELAY_MS = 1500;

    async function refreshJevSettings(): Promise<void> {
      jevSettings = await getJevSettings();
    }

    function buildJevAccount(
      handle: string,
      input: { displayName?: string; text?: string; bio?: string; links?: readonly FeedItemLink[] },
    ): JevAccountInput {
      const metrics = metricsCache.get(handle);
      const hosts = (input.links ?? [])
      ...
      return {
        ...
        // 时间线上只能看到当前这一条推文 —— 线上 state 比离线评测弱，
        // 阈值已按「单条推文」的口径重新标定（judge.py --max-tweets 1 --tag online）。
        ...(input.text ? { tweets: [input.text] } : {}),
        ...(hosts.length > 0 ? { linkHostnames: hosts } : {}),
        ...(metrics ?? {}),
      };
    }

    function enqueueJevJudgment(
      article: HTMLElement,
      handle: string,
      input: { displayName?: string; text?: string; bio?: string; links?: readonly FeedItemLink[] },
      evidence: BlockEvidence,
    ): void {
      if (!isJevActive(jevSettings)) return;
      const account = buildJevAccount(handle, input);
      const key = accountCacheKey(account);
      const cached = jevCache.get(key, Date.now());
      if (cached) {
        // 本会话已经判过同一个内容：直接复用，不再花钱。
        applyJevVerdict(cached, { handle, account, article, evidence });
        return;
      }
      if (jevQueue.has(handle)) return;
      jevQueue.set(handle, { handle, account, article, evidence });
      scheduleJevFlush();
    }

    function scheduleJevFlush(): void {
      if (jevQueue.size >= JEV_BATCH_SIZE) {
        void flushJevQueue();
        return;
      }
      if (jevFlushTimer !== undefined) return;
      jevFlushTimer = window.setTimeout(() => {
        jevFlushTimer = undefined;
        void flushJevQueue();
      }, JEV_FLUSH_DELAY_MS);
    }

    async function flushJevQueue(): Promise<void> {
      if (jevInFlight || jevQueue.size === 0) return;
      jevInFlight = true;
      const batch = [...jevQueue.values()].slice(0, JEV_BATCH_SIZE);
      for (const item of batch) jevQueue.delete(item.handle);
      try {
        const response = (await browser.runtime.sendMessage({
          type: 'feedsieve:jev-judge',
          accounts: batch.map((item) => item.account),
        })) as
          | { ok?: boolean; verdicts?: JevVerdict[]; reason?: string }
          | undefined;
        const verdicts = response?.verdicts;
        if (!response?.ok || !Array.isArray(verdicts)) {
          // 识别层失败必须是静默的：名单/词库照常工作，不打扰用户。
          return;
        }
        const byHandle = new Map(verdicts.map((verdict) => [verdict.handle.toLowerCase(), verdict]));
        const pendingBadges: PendingBadge[] = [];
        for (const item of batch) {
          const verdict = byHandle.get(item.handle.toLowerCase());
          if (!verdict) continue;
          jevCache.set(accountCacheKey(item.account), verdict, Date.now());
          applyJevVerdict(verdict, item, pendingBadges);
        }
        flushPendingBadges(pendingBadges);
      } catch {
        // background 不可达（热重载等）：本轮放弃，下次滚动会重新排队
      } finally {
        jevInFlight = false;
        if (jevQueue.size > 0) scheduleJevFlush();
      }
    }

    function applyJevVerdict(
      verdict: JevVerdict,
      item: JevQueueItem,
      pendingBadges: PendingBadge[] = [],
    ): void {
      // 单元格可能已被 X 复用/移除；也不覆盖用户白名单与关注保护。
      if (!item.article.isConnected) return;
      const handle = item.handle.toLowerCase();
      if (articleHandles.get(item.article) !== handle) return;
      if (allowCache.has(handle) || followingCache.has(handle) || blockedCache.has(handle)) return;

      const thresholds = thresholdsForStrength(strength, jevSettings.thresholds);
      const detection = verdictToDetection(verdict, thresholds);
      if (!detection) return;
      if (pageMarked.has(handle)) return;

      const presentation = classifyDetection({ detection, strength });
      if (presentation === 'ignore') return;

      const cell = item.article.closest(tweetSelectors.timelineCell) ?? item.article;
      const localized = {
        ...detection,
        reason: localizedDetectionReason(uiLanguage, detection),
      };
      markCell(cell as HTMLElement, localized, verdict.category, item.evidence, pendingBadges);
      if (pendingBadges.length > 0) flushPendingBadges(pendingBadges);
    }
