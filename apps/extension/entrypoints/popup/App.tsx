...
export default function App() {
  ...
  useEffect(() => {
    ...
    void getJevSettings().then(setJev);
        ...
        {view === 'settings' ? (
          <div className="view-stack settings-view">
            ...
            <section className="settings-card">
              <div className="settings-card-head">
                <h2>{t.jevTitle}</h2>
                <span className={`community-meta${jev && isJevActive(jev) ? ' is-ready' : ''}`}>
                  {jev && isJevActive(jev)
                    ? t.jevReady
                    : jev?.enabled
                      ? t.jevNotConfigured
              ...
              {jev ? (
                <div className="settings-fields">
                  <label className="setting-row">
                    <span className="setting-copy">
                      <strong>
                        {t.jevEnabled} <HelpIcon text={t.jevEnabledHint} />
                      </strong>
                    </span>
                    <span className="toggle-switch">
                      <input
                        ...
                        checked={jev.enabled}
                        onChange={(event) =>
                          void setJevSettings({ enabled: event.target.checked }).then(setJev)
                        }
                      />
                      <span aria-hidden="true" />
                    </span>
                  ...
                  <div className="setting-block">
                    <div className="setting-copy">
                      <strong>
                        {t.jevApiKey} <HelpIcon text={t.jevApiKeyHint} />
                      </strong>
                    </div>
                    <input
                      type="password"
                      ...
                      onBlur={(event) =>
                        void setJevSettings({ apiKey: event.target.value }).then(setJev)
                      }
                    />
                  </div>
                </div>
