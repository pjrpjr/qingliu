...
**0. Jev 语义判定（默认关闭；开启后会把账号资料发给 TypeSafe）**

默认**关闭**。只有在设置里显式打开「AI 判定（Jev）」并填入你自己的 Typesafe API key 之后，
当名单 / 指纹 / 词库都没认出某个账号时，扩展才会把该账号的**资料文本**发给
`api.typesafe.ai`（TypeSafe AI），换取一个 0–1 的垃圾号概率：

- 发送内容：`handle`、昵称、简介、粉丝/关注/推文数、注册时间、是否默认头像、蓝标，
  以及**时间线上当前可见的推文正文**与外链域名；
- 不发送：你的 X 登录凭证、安装 ID、关注保护名单、浏览历史、私信，以及任何与判定无关的页面内容；
- 请求由扩展的 background 直接发往 TypeSafe，**不经过 FeedSieve 或任何自建服务器**；
- 判定结果缓存在本机（默认 7 天，按账号内容指纹失效），同一账号不会重复发送；
- API key 只存在本机 `browser.storage.local`，不上传；
- 关闭开关后，扩展行为与本政策上一版完全一致（内容不出设备）。
...
**0. Jev semantic verdict (off by default; when on, account text goes to TypeSafe)**

Off by default. Only after you explicitly enable “AI verdict (Jev)” in Settings and enter your own
Typesafe API key does the extension send an account's **profile text** to `api.typesafe.ai`
(TypeSafe AI) — and only when the community list, fingerprints and keyword packs all fail to
recognise it — in exchange for a 0–1 spam probability:

- Sent: `handle`, display name, bio, follower/following/tweet counts, account creation date, whether the avatar is default, verification, plus the **tweet text currently visible on the timeline** and external link hostnames.
- Never sent: your X credentials, installation ID, following-protection list, browsing history, DMs, or any page content unrelated to the verdict.
- Requests go from the extension's background worker straight to TypeSafe — **never through FeedSieve or any self-hosted server**.
- Verdicts are cached locally (7 days by default, invalidated by a content fingerprint) so the same account is not sent twice.
- Your API key stays in local `browser.storage.local` and is never uploaded.
- With the toggle off, the extension behaves exactly as described in the previous version of this policy: content never leaves the device.
